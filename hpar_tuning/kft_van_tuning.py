...
FT_K = 0.5 # paraphraser trained with this k so can not be tuned
FT_USE_BN = False # paraphraser trained with this so can not be tuned
...
def kfold_objective(param, eval_index, folds, full_dataset, num_classes, device, log_dir, 
                    ...
                    tea_logits, tea_preactivation_fmaps, tea_postactivation_fmaps, percentile_prune2=40, paraphraser=None):
    import time
    ...
    translator = Translator(
        stu_model.get_kernel_weights_subset([stu_model.group3indices[-1]])[-1].shape, 
        tea_model.get_kernel_weights_subset([tea_model.group3indices[-1]])[-1].shape, 
        k=FT_K, use_bn=FT_USE_BN).to(device)
    ...
    optimizer_factor = optim.SGD(translator.parameters(), lr=param['factor_lr'], momentum=param['factor_momentum'], weight_decay=0.0)
    ...
    # kd_criterion = FTLoss(translator, paraphraser, stu_model, tea_model, p=param['ft_p'])
    kd_criterion = KFTLoss(translator, paraphraser, stu_model, tea_model, group='group3', p=param['ft_p'])
    alpha = param['alpha']
    gamma = 1.0 - alpha
    beta = param['beta']
...
def main(dataset_name="CIFAR10", requested_device="cuda", runtime='xxx', log_dir="hpar_tuning_logs/vanilla", 
         ...
         student_type='resnet20', teacher_type='resnet56', teacher_path='xxx', use_cached_values=False, disable_vanilla=False, paraphraser_path=None):
    ...
    paraphraser = Paraphraser(teacher_model.get_kernel_weights_subset([teacher_model.group3indices[-1]])[-1].shape, k=FT_K, use_bn=FT_USE_BN).to(device)
        ...
        search: pyhopper.Search = pyhopper.Search(
            {
                ...
                "beta": pyhopper.float(3000.0, 5000.0, init=4000.0),
                "factor_lr": pyhopper.float(0.001, 0.02, log=True, init=0.01),
                "factor_momentum": pyhopper.float(0.91, 0.93, init=0.921, log=True),
                "ft_p": pyhopper.choice([1, 2], is_ordinal=True, init_index=1),
            }
        )
...
if __name__ == "__main__":
    ...
    main(
        ...
        log_dir="hpar_tuning_logs/kernel_factor_transfer",
        name=args.name,
        ...
        paraphraser_path=r'teacher_models\models\resnet56\paraphraser\resnet56_cifar100_73p18.kernel_paraphraser_k_0.5_bn0.pt',
    )