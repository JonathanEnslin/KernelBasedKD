...
def kfold_objective(param, eval_index, folds, full_dataset, num_classes, device, log_dir, 
    ...
    translator = Translator(TARG_SHAPE, TARG_SHAPE, k=FT_K, use_bn=FT_USE_BN).to(device)
    ...
    optimizer_factor = optim.SGD(translator.parameters(), lr=param['factor_lr'], momentum=0.9, weight_decay=5e-4)
...
def main(dataset_name="CIFAR10", requested_device="cuda", runtime='xxx', log_dir="hpar_tuning_logs/vanilla", 
    ...
    paraphraser = Paraphraser(TARG_SHAPE, k=FT_K, use_bn=FT_USE_BN).to(device)
        ...
        search: pyhopper.Search = pyhopper.Search(
            {
                ...
                "beta": pyhopper.float(0.0, 4000.0, init=1000.0),
                "factor_lr": pyhopper.float(0.01, 0.3, log=True, init=0.1),
                "ft_p": pyhopper.choice([1, 2], is_ordinal=True, init_index=0),
            }
        )
...
if __name__ == "__main__":
    ...
    main(
        ...
        log_dir="hpar_tuning_logs/factor_transfer",
        name=args.name,
