...
set "PARAMS1=--params ./params.json --run_name testtest --device cuda --param_set params3 --model_name resnet20 --use_val --val_size 0.1 --disable_test --dataset CIFAR100"
...
@REM     echo Running training run 3, iteration %%j with parameters: %PARAMS3%
@REM     python train.py %PARAMS3%
@REM     if %ERRORLEVEL% neq 0 (
@REM         echo Training run 3, iteration %%j failed.
@REM         exit /b %ERRORLEVEL%
@REM     )
@REM )

@REM for /L %%j in (1,1,%RUN_COUNT%) do (
@REM     echo Running training run 4, iteration %%j with parameters: %PARAMS4%
@REM     python train.py %PARAMS4%
@REM     if %ERRORLEVEL% neq 0 (
@REM         echo Training run 4, iteration %%j failed.
@REM         exit /b %ERRORLEVEL%
@REM     )
@REM )
