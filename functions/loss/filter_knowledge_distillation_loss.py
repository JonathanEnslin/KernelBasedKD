...
class FilterKnowledgeDistillationLoss(nn.Module):
    ...
    def _calculate_filter_loss(self, student_model, teacher_model):
        ...
        aggregated_student_kernels = torch.cat(student_kernels, dim=0)
        aggregated_teacher_kernels = torch.cat(teacher_kernels, dim=0)

        flatten_teacher_kernels = torch.flatten(aggregated_teacher_kernels)
        flatten_student_kernels = torch.flatten(aggregated_student_kernels)
