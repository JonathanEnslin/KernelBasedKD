...
## ============== WARNING ==============
## The implemntations found in the author's repository are not the same as the ones found in the paper.
## Other implementations found also do not match the paper. The modes below represent different implementations
...
class ATLoss(nn.Module):
    def __init__(self, student: BaseModel, teacher: BaseModel, beta=1.0, mode='impl',
        ...
        if mode == 'impl':
            ...
            self.at_map_generator = self._generate_attention_maps_flattened_impl
            self.loss_computer = self._compute_at_loss_impl
        elif mode == 'zoo':
    ...
    def _generate_attention_maps_flattened_impl(self, feature_map, eps=1e-6):
    ...
    def _generate_attention_maps_hobbit(self, feature_map, eps=1e-6):
        # Purely for references purposes, method not used
        return F.normalize(feature_map.pow(2).mean(1).view(feature_map.size(0), -1), eps=eps)
    ...
    def _compute_at_loss_impl(self, student_feature_map, teacher_feature_map):
        at_student = self._generate_attention_maps_flattened_impl(student_feature_map)
        at_teacher = self._generate_attention_maps_flattened_impl(teacher_feature_map)
