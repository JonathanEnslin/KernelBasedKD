...
class ValidationStep(EvaluationStep):
    def __init__(self, model, valloader, criterion, device, writer, start_time, autocast, early_stopping=None, best_model_tracker=None, logger=print, no_write=False):
        super().__init__(model, valloader, criterion, device, writer, start_time, autocast, mode='validation', early_stopping=early_stopping, best_model_tracker=best_model_tracker, logger=logger, no_write=no_write)
