...
class ResNet(BaseModel):
    ...
    def __init__(self, depth, num_classes=1000, block_name='BasicBlock'):
        ...
        if block_name.lower() == 'basicblock':
            assert (depth - 2) % 6 == 0, 'When using basicblock, depth should be 6n+2, e.g. 20, 32, 44, 56, 110, 1202'
            n = (depth - 2) // 6
            block = BasicBlock
        elif block_name.lower() == 'bottleneck':
            assert (depth - 2) % 9 == 0, 'When using bottleneck, depth should be 9n+2, e.g. 20, 29, 47, 56, 110, 1199'
            n = (depth - 2) // 9
            block = Bottleneck
        else:
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                m.register_forward_hook(self._feature_map_hook_fn)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers[-1].conv2.register_forward_hook(self._group_preact_fmap_hook_fn)

        full_layer = nn.Sequential(*layers)
        # register hook
        full_layer.register_forward_hook(self._group_postact_fmap_hook_fn)
        return full_layer
